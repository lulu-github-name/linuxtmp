...
static void arm_timer(struct k_itimer *timer)
{
	...
	if (CPUCLOCK_PERTHREAD(timer->it_clock)) {
		head = p->task_struct_rh->posix_cputimers.cpu_timers + clkidx;
		cpuexp = p->task_struct_rh->posix_cputimers.expiries + clkidx;
	} else {
				...
				struct list_head *firing)
{
	...
	u64 stime, utime, *expires = tsk->task_struct_rh->posix_cputimers.expiries;
	unsigned long soft;
	 ...
	 * CPU timers.
	 */
	if (expiry_cache_is_zero(tsk->task_struct_rh->posix_cputimers.expiries))
		return;
...
{
	u64 *expiries = tsk->task_struct_rh->posix_cputimers.expiries;
