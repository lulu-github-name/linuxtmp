...
static void check_thread_timers(struct task_struct *tsk,
				struct list_head *firing)
{
	struct posix_cputimer_base *base = tsk->task_struct_rh->posix_cputimers.bases;
	unsigned long soft;
	u64 stime, utime;
	...
	if (expiry_cache_is_inactive(&tsk->task_struct_rh->posix_cputimers))
		return;
...
static inline bool fastpath_timer_check(struct task_struct *tsk)
{
	...
	if (!expiry_cache_is_inactive(&tsk->task_struct_rh->posix_cputimers)) {
		...
		if (task_cputimers_expired(samples,
					 &tsk->task_struct_rh->posix_cputimers))
			return true;
	}
